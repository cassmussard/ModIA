...
class vda_river(object):
  ...
  def __init__(self, model_type, param_mesh, param_elements, param_geom, param_bc, param_VDA):
    ...
    #self.C_chol = self.Cov_Expo(1000)
    #self.C_chol = self.Cov_Gaussian(100)
  ...
  def VDA(self,b0,b_t): 
    ...
    k_1st = npl.solve(self.C_chol, (b_1st_np - FEfct_to_array(self.mesh, self.b_b)))
    constr = self.ControlConstraint(k_t, k_1st, Hobs_np)
    #
  ...
  def Direct_Model(self, b) : 
    ...
    # Prepare for Newton-Raphson method for the non-linear problem
    #H = Function(self.V, H0.vector())  # Use the solution of the linear problem as an initial guess
    dH = TrialFunction(self.V)
    Hn, dHn = Function(self.V), Function(self.V)
    ...
    bc_up = DirichletBC(self.V, Constant(0), self.bdry_up) 
    bc_diri_homogene = [bc_up]
    if self.bc_type_out == 'dirichlet':
      bc_down = DirichletBC(self.V, Constant(0), self.bdry_down) 
      bc_diri_homogene.append(bc_down)
    

    # Linearized problem formulation
    lambda_obs = (0.3*(self.Hobs-b) / abs(grad(self.Href)[0]))
    
    a0 = inner(grad(H)[0], grad(lambda_obs * phi)[0])*dx + grad(H)[0]*phi*dx # original linear model
    # RHS of the equation
    L0 =  grad(b)[0]*phi*dx
    # Solving the initial linearized problem
    solve(a0 == L0, Hn, bc_diri)
    ...
    # Non-linear solver loop using Newton-Raphson method
    while error > eps_du and i < i_max:
        i += 1
        
        # Newton step formulation
        lambda_obs = 0.3 * (Hn - b) / abs(grad(self.Href)[0])
        
        # LHS of the linearized variational formulation
        a = inner(grad(dH)[0], phi)*dx+ inner(grad(lambda_obs*phi)[0],grad(dH)[0])*dx+inner(grad(0.3*dH*phi/abs(grad(self.Href)[0]))[0], grad(Hn)[0])*dx
        # RHS of the linearized eqn
        Lan = -inner(grad(Hn)[0], grad(lambda_obs*phi)[0])*dx-grad(Hn)[0]*phi*dx # original linear model
        L = Lan + grad(b)[0]*phi*dx
        # Solve the linear problem in the Newton step
        solve(a == L, dHn, bc_diri_homogene)
        dHn_np = dHn.vector().get_local()
        Hn_np = Hn.vector().get_local()
        
        #Compute error 
        error = np.linalg.norm(dHn_np) / np.linalg.norm(Hn_np)
        Hn.assign(Hn+dHn)
  ...
  def Adjoint_Model(self, b, Hb) : 
    ...
    lambda_obs = 0.3 * (Hb - b) / abs(grad(self.Href)[0])
    a = inner(grad(lambda_obs * p)[0], grad(phi)[0]) * dx+ inner(grad(3 * p * phi / (10 * abs(grad(self.Href)[0])))[0], grad(Hb)[0])*dx  - grad(p)[0]*phi*dx
  ...
  def Cost_np(self,k_array) :
    ...
    if self.jreg_type == 'grad':
      ...
      k = array_to_FEfct(self.V,b_array)
  ...
  def Cov_Expo(self,L):
    ...
    sigma = 1
    for xi in x :
      C[i,:] =  sigma*np.exp(-( np.abs(xp-xi) )/L)
      i +=1
  ...
  def Cov_Gaussian(self,L):
    ...
    sigma = 1
    for xi in x :
      C[i,:] =  sigma*np.exp(-0.5*( (xp-xi)**2 )/L**2) / (np.sqrt(2*np.pi)*L)
      i +=1
