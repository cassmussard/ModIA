...
class vda_river(object):
  ...
  def Direct_Model(self, b) : 
    ...
    if self.bc_type_out == "dirichlet":
        ...
        bc_diri_homo.append(bc_down)

    # LHS of the equation
    lambda_obs = 3 / 10 * (self.Hobs - b) / abs(grad(self.Href)[0])
    a = (
        inner(grad(lambda_obs * phi)[0], grad(H)[0]) * dx + grad(H)[0] * phi * dx
    ...
    i = 0
    i_max = 50
    while error_ > eps_du and i < i_max:
        ...
        a = (
            inner(grad(0.3 * dH * phi / (abs(grad(self.Href)[0])))[0], grad(Hn)[0])
            * dx
            + inner(grad(lambda_obs * phi)[0], grad(dH)[0]) * dx
            + inner(grad(dH)[0], phi) * dx  # Pour stabilité numérique
        )

        L = grad(b)[0] * phi * dx - (
            inner(grad(lambda_obs * phi)[0], grad(Hn)[0]) * dx
            + grad(Hn)[0] * phi * dx
        )  # original linear model

        # Solve
        solve(a == L, dHn, bc_diri_homo)
        # relative diff.
        dun_np = dHn.vector().get_local()
        un_np = Hn.vector().get_local()
        # print(type(dun_np)) #print(dun_np.shape)
        error_ = np.linalg.norm(dun_np) / np.linalg.norm(un_np)

        Hn.assign(Hn + dHn)  # update the solution

        if i % 50 == 0:
            print("Newton-Raphson iteration #", i, "; error = ", error_)
        # test
        if i == i_max:
            print(
                "Warning: the algo exits because of the max number of ite ! error = ",
                error_,
            )
    print(f"iter {i} and eroor {error_}")
    return Hn     
  ...
  def Adjoint_Model(self, b, Hb) : 
    ...
    lambda_obs = 3 / 10 * (Hb - b) / abs(grad(self.Hobs)[0])
    AAA = grad(lambda_obs * phi)[0]
    a = (inner(grad(3 * p * phi / (10 * abs(grad(self.Hobs)[0])))[0], grad(Hb)[0])*dx + inner(AAA, grad(p)[0]) * dx - inner(grad(p)[0], phi) * dx)
