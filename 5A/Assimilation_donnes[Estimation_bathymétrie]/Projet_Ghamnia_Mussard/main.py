...
def run_vda(path) : 
    ...
    #noise = np.convolve(np.random.normal(0, 1, nb_cell), np.ones((50,))/50, mode='same')
    #bathy_1st_np = Hobs_np - 1.5 * prior_depth + noise
    ##Ajout d'un sinus 
    #omega = 2 * np.pi / L
    #bathy_1st_np = Hobs_np - 0.75 * prior_depth + 0.2 * prior_depth * np.sin(omega * np.linspace(0, L, nb_cell))
    ...
    covar_matrix = 1 # MAY BE CHANGED
    if covar_matrix == 1 :
        L = 100 # MAY BE CHANGED
        #C_chol = my_vda.Cov_Expo(L)
        C_chol = my_vda.Cov_Gaussian(L)
        #C_chol = np.eye(param_mesh['number cells']) 
