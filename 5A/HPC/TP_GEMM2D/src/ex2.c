...
void bcast_A(int p, int q, Matrix *A, int i, int l, MPI_Comm row_comm) {
    ...
    if (q > 1 && Ail->row ==my_row /* Ail is stored on my row */) {
      if (Ail->owner != me) {
...
void bcast_B(int p, int q, Matrix *B, int l, int j, MPI_Comm col_comm) {
    ...
    if (p > 1 && Blj->col ==my_col /* Blj is stored on my column */) {
      if (Blj->owner != me) {
...
//Vert broadcast
//BLeu envoie de donnée
//Rouge calcul du check
//Flèche = envoie de donnée
